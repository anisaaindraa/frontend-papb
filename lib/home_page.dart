...
class _HomePageState extends State<HomePage> {
  ...
  // Fungsi untuk menyimpan kata favorit ke Firestore
  Future<void> _updateFavoritesInFirestore(Set<String> favorites) async {
    final User? user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      // Mendapatkan referensi dokumen berdasarkan UID pengguna
      final userFavoritesRef =
          FirebaseFirestore.instance.collection('userFavorites').doc(user.uid);
      await userFavoritesRef.set({
        'favorites': favorites.toList(),
      }, SetOptions(merge: true)); // Gunakan merge agar tidak menimpa data lain
    }
  }
            ...
            IconButton(
              ...
              onPressed: () {
                setState(() {
                  ...
                  // Update Firestore with the new set of favorites
                  _updateFavoritesInFirestore(favoriteWords);
                });
              },
            ),
          ],
